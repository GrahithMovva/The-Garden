...
export class UserRequestService {
  ...
  private requestsURL = 'http://localhost:8080/requests';  // URL to web api
  ...
  getPlants(): Observable<Plant[]> {
    return this.http.get<Plant[]>(this.requestsURL)
      .pipe(
  ...
  getPlantNo404<Data>(id: number): Observable<Plant> {
    const url = `${this.requestsURL}/?id=${id}`;
  ...
  searchPlants(term: string): Observable<Plant[]> {
    ...
    return this.http.get<Plant[]>(`${this.requestsURL}/?name=${term}`).pipe(
      tap(x => x.length ?
  ...
  addPlant(plant: Plant): Observable<Plant> {
    return this.http.post<Plant>(this.requestsURL, plant, this.httpOptions).pipe(
  ...
  updatePlant(plant: Plant): Observable<any> {
    return this.http.put(this.requestsURL, plant, this.httpOptions).pipe(
